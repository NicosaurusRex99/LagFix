...
public class CommandNukeNonAnimal implements ICommand {
  ...
  public List getAliases() {
    return Arrays.asList(new String[] {"nukenonanimals"});
  }
  ...
  public void execute(ICommandSender icommandsender, String[] params) {
      ...
      for ( int k = 0; k < world.loadedEntityList.size(); k++ ) {
        ...
        if (!( it instanceof EntityPlayer )) {
          if ( (it.posX<=(px+range)) && (it.posX>=(px-range)) && (it.posZ<=(pz+range)) && (it.posZ>=(pz-range)) ) {   
            if ( (!( it instanceof EntityAnimal )) && ( it instanceof EntityLiving) ) { 
      ...
      Do.Say(player, "§e"+ itcount + "§r Living but non-animals removed in range +-"+range);
  }
